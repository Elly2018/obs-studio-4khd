	...
	: OBSMainWindow(parent), undo_s(ui), ui(new Ui::OBSBasic)
{
	...
	connect(App(), &OBSApp::StyleChanged, this, &OBSBasic::ResetProxyStyleSliders);
...
bool OBSBasic::LoadService()
{
	...
	service2 = obs_service_create(type, "default_service", settings,
				     hotkey_data);
	//obs_service_set_key(service2, nullptr);
...
bool OBSBasic::InitService()
{
	...
	service = obs_service_create("rtmp_common", "default_service", nullptr,
				     nullptr);
	service2 = obs_service_create("rtmp_common", "default_service", nullptr,
				     nullptr);
	if (!service)
		return false;
	obs_service_release(service);
...
void OBSBasic::ResetOutputs()
{
	...
	if (!outputHandler2 || !outputHandler2->Active()) {
		outputHandler2.reset();
		outputHandler2.reset(advOut ? CreateAdvancedOutputHandler(this)
					    : CreateSimpleOutputHandler(this));

		delete replayBufferButton;

		if (outputHandler2->replayBuffer) {
			replayBufferButton = new ControlsSplitButton(
				QTStr("Basic.Main.StartReplayBuffer"),
				"replayBufferButton",
				&OBSBasic::ReplayBufferClicked);
		...
		if (sysTrayReplayBuffer)
			sysTrayReplayBuffer->setEnabled(
				!!outputHandler2->replayBuffer);
	} else {
		outputHandler2->Update();
	}
}
...
#undef SET_VISIBILITY
	...
	connect(ui->streamButton, &QPushButton::clicked, this,
		&OBSBasic::on_streamButton_clicked);
	connect(ui->streamButton2, &QPushButton::clicked, this,
		&OBSBasic::on_streamButton_clicked2);
...
#define MAKE_CALLBACK(pred, method, log_action)                            \
	...
	streamingHotkeys2 = obs_hotkey_pair_register_frontend(
		"StartStreaming2", "Streaming Second",
		"StopStreaming2", "Streaming Second",
		MAKE_CALLBACK(!basic.outputHandler2->StreamingActive() &&
				      basic.ui->streamButton2->isEnabled(),
			      basic.StartStreaming2, "Starting Second Stream"),
		MAKE_CALLBACK(basic.outputHandler2->StreamingActive() &&
				      basic.ui->streamButton2->isEnabled(),
			      basic.StopStreaming2, "Stopping Second Stream"),
		this, this);
	LoadHotkeyPair(streamingHotkeys2, "OBSBasic.StartStreaming2",
		       "OBSBasic.StopStreaming2");
...
void OBSBasic::ClearHotkeys()
{
	...
	obs_hotkey_pair_unregister(streamingHotkeys2);
...
void OBSBasic::StartStreaming2()
{
	blog(LOG_INFO, "YOOOOO, It's 2 streaming time !!");
	if (outputHandler2->StreamingActive())
		return;
	if (disableOutputsRef)
		return;

	outputHandler2->SetSuffix("2");
	if (!outputHandler2->SetupStreaming(service2)) {
		DisplayStreamStartError();
		return;
	}
	...
	ui->streamButton2->setEnabled(false);
	ui->streamButton2->setChecked(false);
	ui->streamButton2->setText(QTStr("Basic.Main.Connecting"));

	if (sysTrayStream) {
		sysTrayStream2->setEnabled(false);
		sysTrayStream2->setText(ui->streamButton2->text());
	}

	if (!outputHandler2->StartStreaming(service2)) {
		DisplayStreamStartError();
		return;
	}
...
void OBSBasic::StopStreaming2()
{
	SaveProject();

	if (outputHandler->StreamingActive())
		outputHandler->StopStreaming(streamingStopping2);
...
void OBSBasic::ForceStopStreaming2()
{
	SaveProject();
...
{
	ui->streamButton2->setText(QTStr("Basic.Main.StopStreaming"));
	ui->streamButton2->setEnabled(true);
	ui->streamButton2->setChecked(true);

	if (sysTrayStream) {
		sysTrayStream->setText(ui->streamButton2->text());
		sysTrayStream->setEnabled(true);
	}
	...
	startStreamMenu->addAction(QTStr("Basic.Main.StopStreaming"), this,
				   SLOT(StopStreaming2()));
	startStreamMenu->addAction(QTStr("Basic.Main.ForceStopStreaming"), this,
				   SLOT(ForceStopStreaming2()));
	ui->streamButton2->setMenu(startStreamMenu);
...
{
	ui->streamButton2->setText(QTStr("Basic.Main.StartStreaming"));
	ui->streamButton2->setEnabled(true);
	ui->streamButton2->setChecked(false);
...
{
	ui->streamButton2->setText(QTStr("Basic.Main.StoppingStreaming"));

	if (sysTrayStream)
		sysTrayStream->setText(ui->streamButton2->text());

	streamingStopping2 = true;
	if (api)
		api->on_event(OBS_FRONTEND_EVENT_STREAMING_STOPPING);
}
...
void OBSBasic::StreamingStop2(int code, QString last_error)
{
	...
	if (!startStreamMenu.isNull()) {
		ui->streamButton2->setMenu(nullptr);
		startStreamMenu->deleteLater();
		startStreamMenu = nullptr;
	}
...
void OBSBasic::on_streamButton_clicked2()
{
	outputHandler2->SetSuffix("2");
	if (outputHandler2->StreamingActive()) {
		bool confirm = config_get_bool(GetGlobalConfig(), "BasicWindow",
		...
		if (this->auth) {
			OBSDataAutoRelease settings =
				obs_service_get_settings(service2);
			bwtest = obs_data_get_bool(settings, "bwtest");
			// Disable confirmation if this is going to open broadcast setup
			if (auth && auth->broadcastFlow() && !broadcastReady &&
			    !broadcastActive)
...
void OBSBasic::SystemTrayInit()
{
	...
	sysTrayStream = new QAction(
		...
		trayIcon.data());
	sysTrayStream2 = new QAction(
		StreamingActive2() ? QTStr("Stop Stream 2")
				  : QTStr("Start Stream 2"),
		trayIcon.data());
	...
	trayMenu->addAction(sysTrayStream2);
	...
	connect(sysTrayStream2, SIGNAL(triggered()), this,
		SLOT(on_streamButton_clicked2()));
...
bool OBSBasic::StreamingActive2()
{
	if (!outputHandler2)
		return false;
	outputHandler2->SetSuffix("2");
	return outputHandler2->StreamingActive();
}
